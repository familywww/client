<template>
  <v-main :class="moveMarkerClassName">
    <div style="position:absolute;top:0px;left:10px;background:#00000000;z-index:500;">
      <v-btn-toggle v-model="map.index" style="background:#2b3575a8;margin-right: 10px">
        <v-btn v-for="(subItem, j) in map.tile" :key="j" :value="j" style="background:#2b3575a8;color:white">{{ subItem.name }}</v-btn>
      </v-btn-toggle>
      <v-btn-toggle >
        <v-icon color="#00c5f5" @click="setZoom(0)">mdi-plus-thick</v-icon>
        <v-icon color="#00c5f5" @click="setZoom(1)">mdi-minus-thick</v-icon>
        <span style="margin: 0 5px">{{ zoomVal }}</span>
      </v-btn-toggle>
    </div>
    <l-map
      ref="map"
      ...
      :crs="map.tile[map.index].crs"
      :options.sync="map.options"
      @move="zoomEnd"
    >
      ...
      <l-tile-layer
        v-for="layer in map.tile[map.index].maps"
        :key="layer.name"
        :name="layer.name"
        :visible="layer.visible"
        :url="layer.url"
        :options="{tms: map.tile[map.index].tms, zIndex: layer.zIndex, opacity: layer.opacity}"
        :subdomains="map.tile[map.index].subdomains"
    ...
    <v-card width="240" style="position:absolute;height:100%;top:0px;right:0px;z-index:500;background:#2b3575a8">
      <v-card-title>
      ...
      <v-list dense class="overflow-y-auto" max-width="240" style="position:absolute;width:100%;height:calc(100% - 160px);background: #2b357500">
    ...
    <video
      ref="videoFlv"
      style="position:absolute;right:250px;bottom:10px;width:240px;height:135px;z-index:500;"
      autoplay
      loop
      controls
    ...
    <div ref="videoJs" style="position:absolute;right:500px;bottom:10px;width:240px;height:135px;z-index:500">
      <video
        id="videoJs"
    ...
    <canvas ref="canvas" style="position:fixed;top:0;left:0;z-index: 901;pointer-events:none"/>
  </v-main>
</template>
...
export default {
    ...
    return {
      ...
      markers: [
        ...
        },
        {
          ...
          type: 'point',
          children: [
            { id: '百度天安门城楼', lng: 116.403964, lat: 39.915099, status: 's_01', videoItem: { name: 'video/mp4', type: 'video/mp4', src: 'https://mp4.vjshi.com/2019-03-23/3b0aace78bc58d7f866cdbe494581e3f.mp4', }, },
  ...
  created () {
    this.createMarkerOption()
    this.createMarkerObj(73)
    this.createMarkerIcon(this.markers)
  },
  mounted () {
    this.videoInit()
    this.globalMouseEvent()
  },
  methods: {
    globalMouseEvent () {
      const self = this
